...
module.exports.legacyCheck = (env = cds.env) => {
  ...
  if ([...LINTER_OPTIONS, ...LEGACY_OPTIONS].some(option => compat?.[option])) {
    LOG.warn(`Warning: Legacy configuration 'mtx' is used. Please update to the new linter options. See ${capire} for details.`)
  }

  for (let p of LEGACY_OPTIONS) if (compat?.[p] || conf?.[p]) {
    LOG.warn(`Warning: Legacy option '${p}' is used. Please update to the new linter options: ${LINTER_OPTIONS.join(', ')}. See ${capire} for details.`)
  }
}
...
module.exports.lint = (extCsn, fullCsn, env = cds.env, cloneExt = false) => {
  ...
  let x
  for (let p of LINTER_OPTIONS) if ((x = conf[p] || compat[p])) {
    linter_options[p] = x
  }
  for (let p of LEGACY_OPTIONS) if ((x = compat[p])) {
      linter_options[p] = x
      LOG.warn(`Warning: Legacy option '${p}' is used. Please update to the new linter options: ${LINTER_OPTIONS.join(', ')}. See ${capire} for details.`)
  }
